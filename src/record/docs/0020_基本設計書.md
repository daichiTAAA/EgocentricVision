...
| GET          | /api/v1/streams/debug            | GStreamerパイプライン状態を含む詳細なデバッグ情報を取得する。 |
...
#### **ストリームデバッグ情報取得 (GET /api/v1/streams/debug)**

* **説明**: GStreamerパイプラインの状態を含む詳細なデバッグ情報を取得する。開発時やトラブルシューティング時に使用する。
* **リクエストボディ**: なし
* **レスポンス (200 OK)**:
  ```json
  {
